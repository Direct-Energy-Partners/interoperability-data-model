# Circuit Breaker schema reference

Generated from `schema/breaker.json` by `scripts/gen_schema_docs.py`. The schemas are the source of truth (dcide-app validators). Do not edit this page by hand.

- Type key: `breaker`
- Indicator: `Q`
- Required top level fields: `name`
- Example instance: [`examples/testBreaker.json`](../examples/testBreaker.json)
- Narrative guide: [Circuit Breaker](../breaker.md)
...
| `viewedCount` | number |  |  |  |
| `id` | string |  |  |  |
| `canServeAs` | array |  | `[]` | items: one of `battery`, `bus`, `capacitor`, `charger`, `breaker`, `controller`, `combinerBox`, `contactor`, ... (39 total) |
| `electrical` | object |  | `{}` |  |
