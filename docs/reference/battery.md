# Battery schema reference

Generated from `schema/battery.json` by `scripts/gen_schema_docs.py`. The schemas are the source of truth (dcide-app validators). Do not edit this page by hand.

- Type key: `battery`
- Indicator: `BAT`
- Required top level fields: `name`
- Example instance: [`examples/testBattery.json`](../examples/testBattery.json)
- Narrative guide: [Battery](../battery.md)

## Top level attributes

| Field | Type | Required | Default | Allowed / notes |
| --- | --- | --- | --- | --- |
| `type` | const |  | `battery` | = `battery` |
| `name` | string | yes |  |  |
| `description` | string |  | `""` |  |
...
| `energyCapacity` | measurement |  | `{"unit": "J"}` | fields: value; unit `J` |
| `chargeCapacity` | measurement |  | `{}` | fields: value; unit `C` |
| `batteryTechnologies` | array |  | `[]` | items: one of `li-ion`, `lfp`, `lto`, `nimh`, `nicd`, `lead-acid`, `flow`, `solid-state`, ... (12 total) |
| `standards` | array of string |  | `[]` |  |
...
| `powerFlowDirection` | const |  | `bidirectional` | = `bidirectional` |
| `terminal` | object |  | `{}` |  |
| `wireSize` | object |  | `{}` |  |
| `DC` | object |  | `{}` |  |
