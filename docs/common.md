...
- `teamManufacturer` (string, nullable): team scoped manufacturer reference.
- `distributors` (string array): distributor references.
- `distributorsDetails` (array): per distributor objects with `distributor` (string) and
  optional `price` (number).
- `productIdentifier` (string): manufacturer part number or SKU.
- `productSeries` (string): product family or series name.
- `msrp` (number, nullable): manufacturer suggested retail price.
- `leadTime` (number, nullable): lead time in days.
- `lifecycle` (object):
...
- `regionAvailability` (array): subset of `EU`, `US`, `Canada`, `China`, `Japan`.
...
- `questions` (array): objects with `question` (string) and `answer` (string).
...
- `files` (array): each entry has `file` (id or object), `type` (component file type) and
  optional `visibility` (`public` or `private`, default `public`).
- `images` (array): each entry has `file` (id or object, nullable) and `type` (one of
  `thumbnail`, `ISOPicture`, `front`, `rear`, `left`, `right`, `top`, `bottom`, `general`).
  New components start with one empty slot per standard view.

## Metadata and compatibility

- `metadata` (object): free form per field annotations. Each field key may carry `display`
  (`na`, `unknown`, `hidden`) and `notes` (string).
- `compatibleWith` (string array, nullable): components known to be compatible.
- `compatibleWithCrossReference` (string array, nullable): derived cross references.
- `compatibleWithPlaceholders` (array): unresolved compatibility entries with `manufacturer`,
  `productIdentifier` and optional `componentType`.

## System fields

These are managed by the platform and appear in stored instances.

- `id` (string): assigned on creation.
- `visibility` (`private` or `public`, default `private`).
- `reviewed` (boolean), `publishedBy` (string, nullable), `publishedAt` (date, nullable).
- `archivedAt`, `deletedAt` (date, nullable).
- `completeness`, `viewedCount` (number, optional).
