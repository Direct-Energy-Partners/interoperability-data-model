...
The schemas are generated from the dcide-app component validators, which are the source of truth, and cover the 31 component types that appear in the product catalog. Each example is produced from the same validators and is valid against its schema. See `docs/common.md` for the shared attribute base (identification, compliance, communication, environmental, mechanical, performance, files, images and metadata) and for the shared port model with its AC and DC blocks.
